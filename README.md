...
## Example Grids
The grids that can be found in folder `grids` have been downloaded from [Sudoku 9x9](https://sudoku9x9.com/), which allows to solve grids with 5 different levels (named **beginner**, **elementary**, **intermediate**, **expert** and **master** in the folder).
The repository also contain 2 python scripts:
- `get_grids.py` allows to download 10 additional grids for each level from [Sudoku 9x9](https://sudoku9x9.com/), and stores them in folder `grids`, assigning the files where they are stored increasing sequential indices.
- `launch_grids.py` executes the solver with all the grids stored in folder `grids` that have one of the levels listed above as filename prefix, also storing the output of every execution in folder `logs`
